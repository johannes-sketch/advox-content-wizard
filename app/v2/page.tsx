...
async function handleMagicFlow() {
  if (!topicInput) return alert("Bitte Thema eingeben!");
  setIsMagicLoading(true);
  try {
    const res = await fetch("/api/magic", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt: prompt }), // Nutzt den strategischen advox-Prompt
    });
    if (res.ok) alert("🚀 Erfolg! Content generiert und im Sheet gespeichert.");
    else alert("Fehler beim Speichern.");
  } catch (err) {
    alert("Fehler im Ablauf.");
  }
  ...
  style={{
    ...
    color: "white", padding: "15px 25px", borderRadius: "10px", fontWeight: "bold"
  }}
>
  {isMagicLoading ? "Zaubere..." : "1-Klick: Generieren & Speichern 🪄"}
</button>
